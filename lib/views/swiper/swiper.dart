...
class SwiperView extends ViewModelWidget<SwiperModel> {
  ...
  Widget render(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          title: Text(MyRoutes.swiper.title),
        ),
        body: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text("上传轮播图到后台以后，复制File ID填入下面文本框，多个轮播图换行隔开",
            ...
            Container(
                ...
                child: TextField(
                  ...
                  decoration: const InputDecoration(
                      hintText: "输入轮播图File ID", labelText: "轮播图Urls"),
                )),
            Padding(
                padding: const EdgeInsets.all(6),
                child: FilledButton(
                    onPressed: () => {viewModel.onPressSubmit(context)},
                    child: const Text("设置")))
          ],
        ));
  }
